<template>
    <div class="fixed top-0 left-0 h-full w-full overflow-x-hidden z-30" ref="maindiv">
        <div class="tiltleft z-20 absolute top-0 h-full w-9/12 bg-gradient-to-bl from-torange to-tdred flex items-center justify-end border-none" ref="left">
            <div class="flex flex-col">
                <h2 class="font-redressed text-white text-4xl">Susseum Loading.</h2>
                <LoadingBar :height="1" :width="14" :percent="percent"/>
            </div>
        </div>
        <div class="tiltright z-20 absolute top-0 h-full w-9/12 bg-gradient-to-tl from-tdred to-torange border-none" ref="right"></div>
    </div>
</template>
    ...
    methods:{
        ...
        loaded(){
            ...
            gsap.to(this.$refs.right, { x:window.innerWidth/1.8, ease: "power1.inOut", duration: 1.2,  onComplete: ()=>{
                ...
                this.$refs.maindiv.classList.toggle("z-30")
